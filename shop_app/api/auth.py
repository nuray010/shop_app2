...
from shop_app.database.schema import UserProfileOutSchema, UserProfileInputSchema, UserLoginSchema
...
async def register(user: UserLoginSchema, db: Session = Depends(get_db)):
    user_db = db.query(UserProfile).filter(UserProfile.user_name==user.username).first()
    if not user_db or not verify_password(user.password, user_db.password):
        raise HTTPException(detail='Сиз жазган маалымат туура эмес', status_code=401)

    access_token = create_access_token({'sub': user_db.user_name})
    refresh_token = create_refresh_token({'sub': user_db.user_name})
...
async def logout(refresh_token: str, db: Session = Depends(get_db)):
   ...
   return {"message": "вы вышлииииииии"}
